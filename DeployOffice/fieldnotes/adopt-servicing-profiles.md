...
- Like other cloud services or Microsoft Configuration Manager, servicing profile is an asynchronous service. When you create or change the configuration, the service will process your input in the background. The user interface (especially the **Devices** tab) won't reflect your changes immediately. 
...
The portal will receive status information about these steps and the admin will see devices transitioning from **Not Started** to **In Progress** and to **Updated** finally. If an error occurs on the device, it will be flagged accordingly in the portal and the update applied again. In most cases failures are related to the download of the update; for example, when the device was shut down while the download was active.
