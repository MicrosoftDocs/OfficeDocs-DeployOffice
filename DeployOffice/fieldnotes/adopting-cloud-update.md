...
- **Increased reach**: Common update management tools are often limited by the boundary of a directory service. To manage devices, those must be joined into Active Directory and/or Microsoft Entra ID. With cloud update, you're breaking this barrier. Cloud update manages all Microsoft 365 Apps instances which have an Microsoft Entra user from your tenant signed in. It doesn't matter if the device is joined into Active Directory, Microsoft Entra ID, registered into Microsoft Entra ID, or remains in workgroup mode.
