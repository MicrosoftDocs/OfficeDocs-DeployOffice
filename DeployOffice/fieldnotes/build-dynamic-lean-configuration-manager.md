...
- Deploy an application that instruct the device to [perform a channel change](../change-update-channels.md#change-the-update-channel-with-configuration-manager) to Monthly Enterprise Channel as "available" to the collection that holds SAEC and SAEC-P devices. This enables users to switch the channel on their own terms and at their own pace. You'd see the devices transitioning to the new collection automatically.
