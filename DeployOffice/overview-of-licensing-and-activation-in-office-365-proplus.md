...
If the user hasn't been assigned a license, and they try to use Office 365 ProPlus on a computer where it's installed, Office will be in reduced functionality mode. Also, the user will be prompted to sign in and activate every time they open an Office app, such as Word or Excel.
 ...
 ## ViewerMode for Office 365 & Microsoft 365 subscribers.

Starting April 1, 2020 customers that have an Office 365 or Microsoft 365 license may use Office 365 ProPlus, Project, and Visio in ViewerMode.  This is especially helpful if your organization configures multiple deployment packages for users based on their licensing assignment. With ViewerMode you can set the Group Policy for the specified users and the users are permitted to open  files in a "Read" and "Print" only mode called ViewerMode. This requires Office version 1903 and the above Group Policy to be enabled in the organization unit(OU). 
