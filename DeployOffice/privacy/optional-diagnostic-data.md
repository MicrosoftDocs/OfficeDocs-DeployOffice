...
> [!NOTE]
> If you're using a version of Office that doesn't give you the ability to choose whether to send us required or optional diagnostic data, then only required diagnostic data is sent. For example, if you're using Office Professional Plus 2019 or Office Professional 2013, which don't provide that choice, then only required diagnostic data is sent. For more information about which versions of Office provide this choice, see [Privacy controls available for Office products](products-versions-privacy-controls.md).
