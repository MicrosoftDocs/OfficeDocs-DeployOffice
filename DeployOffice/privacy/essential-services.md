...
## Experimentation and Configuration Service (ECS) events
...
If we end up closing/crashing office due to a licensing issue, we send out this signal to indicate the same. It is critical in detecting if the user is in a good state and not missing functionality, used for system health and used for diagnostic purposes if a user reports an issue with their machine.
...
No necessary service diagnostic data events are collected by Services Configuration.
