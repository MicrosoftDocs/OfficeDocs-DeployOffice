...
This specifies which update channel Microsoft AutoUpdate receives updates from.
...
| Comments | Not having this entry, or having an invalid entry in ChannelName, will revert MAU to the default 'Current' channel.<br>Following values are deprecated and should no longer be used:<br>• External (replaced by Preview)<br>• InsiderFast (replaced by Beta)<br>• Production (replaced by Current)<br>Note: ‘Custom’ is provided to help facilitate hosting updates within an organization, to reduce external network traffic. In such a case, IT Admin is responsible for following:<br>• Manage server space within the organization<br>• Downloading manifests to specified manifest server location<br>• Downloading update packages to specified update cache location (if applicable) |
...
| Manageable | Yes |
| Accepted Values | TRUE / FALSE / 1 / 0 |
| Comments | Not having this entry, or setting this to FALSE will result in Opt-In popup window showing if HowToCheck is set to Automatic.<br>Should be ignored if HowToCheck setting is NOT Automatic. |
...
When set TRUE, this used to disable the pulldown menu to select Beta/Preview channels.
...
When set FALSE, this disables the option to initiate check for updates from the MAU GUI.
...
| Default Value | |
| Manageable | Yes |
| Comments | Should be set to enforce updates by a certain date.<br>Example:<br>`<dict>`<br>    `<key>path to the application</key>`<br>    `<dict>`<br>        `<key>"Application ID"</key>`<br>        `<String>OPIM2019</String>`<br>        `<key>"ForcedUpdateDate"</key>`<br>        `<Date>2024-04-01</Date>`<br>        `<key>"ForcedUpdateVersion"</key>`<br>        `<String>16.84.24032800</String>`<br>    `</dict>`<br>`</dict>` |
...
| Comments | Setting this to None will cause Full updaters to be downloaded, impacting network usage.<br>Unless there is a reason to use Full updater, this should be left empty, or set to Size. |
...
| Manageable | Yes |
| Accepted Values | TRUE / FALSE / 1 / 0 |
| Comments | Use this if your organization uses a security app that modifies installed apps. |
