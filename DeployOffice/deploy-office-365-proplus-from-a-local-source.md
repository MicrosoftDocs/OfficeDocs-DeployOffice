...
## Step 2: Create a configuration file for the broad group
  
To download and deploy Office 365 ProPlus for the broad group , we'll create a configuration file that defines the appropriate settings for the ODT.
...
This configuration file is used to download Office 365 ProPlus installation files and then deploy them to the broad group. Here's more detail on how the XML settings define what to download and what to install:
...
This configuration file is used to download Office installation files and then deploy them to the pilot group. The settings are exactly the same as the first configuration file, except the source path points to a different folder (SACT), and the installation channel and update channel are both set to Semi-Annual Channel (Targeted).
...
## Step 5: Download the Office installation package for the broad group
...
## Step 7: Deploy Office 365 ProPlus

To deploy Office, we'll provide two commands that users can run from their client computers. The first command installs Office for client computers in the broad group and the second for the pilot group. The commands run the ODT in configure mode and with a reference to the appropriate configuration file, which defines which version of Office to install on the client computer.
...
**Run the command for the broad group**
  
From the client computers for the broad group, run the following command from a command prompt with admin privileges:
  
 `\\Server\Share\O365\setup.exe /configure \\Server\Share\O365\config-group1-SAC.xml`
  
**Step 2: Run the command for the pilot group**
...
This command is the same as the broad group, except that it references the configuration file for the pilot group.
...
## Step 8: Verify that the installation was successful
