...
By default update exclusions will affect all devices which are in scope of the servicing profile. You can also select to scope the exclusion window to Azure AD groups. Then only devices or users specified in the Azure AD group will be excluded, all other devices will follow the regular update deployment rythm. Check out this video for a quick overview of [Azure AD group scoping with update exclusion windows](https://youtu.be/WMVlfg_3wnw).
