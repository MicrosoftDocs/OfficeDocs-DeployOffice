...
- You can use the RemoveMSI element in conjunction with the MSICondition attribute. That attribute allows you to control the installation of an Office product based on whether an MSI-based product is already installed on the device. For example, only install Microsoft 365 Apps for enterprise if Office Standard is already installed on the device. For more information, see [MSICondition attribute)](office-deployment-tool-configuration-options.md#msicondition-attribute-part-of-product-element).
