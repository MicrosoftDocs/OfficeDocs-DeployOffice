...
In managed environments, each of these items would require a dedicated installation package for automatic, controlled installation for a user. Usually, an admin would combine the necessary source files (of ~2.5 gigabytes) and a copy of the Office Deployment Tool (ODT) together with a configuration file into a package for each of these components.

But, especially in larger organizations, you often don't run a single installation of Office 365 ProPlus. You might have a mix of update channels (often SAC and SAC-T). And maybe you're currently transitioning from 32-bit to 64-bit, and you'll have to support both architectures for quite some time.

So at the end, you wouldn't have *1* package per component but *4*, covering each possible permutation of SAC/SAC-T and x86/x64. The end result would be:
...
- Low satisfaction for users who have to pick their package from among numerous options.
...
You can resolve all these issues by implementing self-adjusting, small, and universal packages. Let's cover the basic concepts before we dive into sample scenarios.
...
- Instead of pushing a 2.5-GB install package to clients, we allow clients to pull what they need on demand from Office CDN, which saves bandwidth:
   - When you Project to an existing Office 365 ProPlus installation, you need to download less than 50 megabytes, as Office shared components are already installed.
...
- A second install scenario is often less frequent, which lowers the burden on the internet traffic, ultimately reducing the impact.
...
The idea behind this concept is to not hard code everything in the configuration file, but to instead utilize the cleverness of the Office Deployment Tool (ODT) as much as possible.

Let’s have a look at a “classic” package, which was built to add Project to an existing install of Office 365 ProPlus. We have the source files (of ~2.5 gigabytes) and a configuration file, which explicitly states what we want to achieve:
...
- We added *Language ID="MatchInstalled"*  and *TargetProduct* to match the currently installed languages, replacing a hard-coded list of languages to install.
   ...
   - Benefit III: Also works for rarely used languages that you as central IT admin don’t offer, which makes user happy.
...
If you’re running SAC as well as SAC-T and have a x86/x64 mixed environment, you'd need three additional files to cover the remaining configuration permutations. Or, you just go the dynamic, lean, and universal way:
